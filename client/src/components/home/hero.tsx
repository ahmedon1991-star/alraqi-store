...
export function Hero() {
  return (
    <div className="relative w-full h-[450px] md:h-[800px] overflow-hidden flex items-center bg-background border-b border-primary/5 transition-colors duration-500">
      {/* Background with Radial Glow and Elements */}
      <div className="absolute inset-0 z-0">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_50%_40%,_rgba(200,150,62,0.12),_transparent_60%)]" />
        <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-primary/30 to-transparent" />
        
        {/* Floating Particles (Simulated) */}
        <div className="absolute h-full w-full pointer-events-none opacity-40">
           <div className="absolute top-1/4 left-1/4 h-3 w-3 rounded-full bg-primary/40 animate-ping-slow" />
           <div className="absolute top-3/4 right-1/3 h-2 w-2 rounded-full bg-primary/30 animate-pulse" />
           <div className="absolute top-1/3 right-1/4 h-4 w-4 rounded-full bg-primary/20 animate-ping-slow delay-700" />
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="flex flex-col items-center text-center max-w-4xl mx-auto space-y-6 md:space-y-10 animate-fade-up">
          ...
          <div className="space-y-4">
             <h1 className="text-5xl md:text-[140px] font-display font-black leading-[0.9] text-foreground drop-shadow-[0_10px_20px_rgba(0,0,0,0.1)] dark:drop-shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-6 w-full justify-center px-8 sm:px-0">
            <Link href="/shop" className="w-full sm:w-auto">
              <Button size="lg" className="h-14 md:h-24 w-full sm:px-20 rounded-2xl md:rounded-[2.5rem] bg-gradient-to-br from-primary to-primary/80 text-primary-foreground font-heading font-black text-lg md:text-3xl shadow-[0_15px_45px_rgba(200,150,62,0.3)] hover:shadow-[0_20px_60px_rgba(200,150,62,0.5)] hover:scale-105 transition-all duration-500 group">
                تسوق المجموعة
            ...
            <Link href="/shop?offers=true" className="w-full sm:w-auto">
              <Button variant="outline" size="lg" className="h-14 md:h-24 w-full sm:px-16 rounded-2xl md:rounded-[2.5rem] border-primary/40 bg-card/10 backdrop-blur-md text-foreground font-heading font-black text-lg md:text-2xl hover:bg-card/20 hover:border-primary transition-all duration-500">
                العروض الذهبية
              </Button>
            </Link>
          </div>
